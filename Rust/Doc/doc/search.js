(function () {
  ...
  window.initSearch = function (rawSearchIndex) {
    ...
    function execQuery(query, searchWords, filterCrates) {
      ...
      function checkType(obj, val, literalSearch) {
        ...
        if (lev_distance <= MAX_LEV_DISTANCE) {
          lev_distance = Math.ceil(
            (checkGenerics(obj, val) + lev_distance) / 2
          );
        }
        if (obj.length > GENERICS_DATA && obj[GENERICS_DATA].length > 0) {
        ...
        if (filterCrates !== undefined) {
          ...
          Object.keys(ALIASES).forEach(function (crate) {
            if (ALIASES[crate][query.search]) {
              var pushTo =
                crate === window.currentCrate ? crateAliases : aliases;
      ...
      if (
        ...
        for (i = 0; i < nSearchWords; ++i) {
          ...
          if (
            ...
            results[fullId] = { id: i, index: -1, dontValidate: true };
          }
          if (in_args && results_in_args[fullId] === undefined) {
            results_in_args[fullId] = { id: i, index: -1, dontValidate: true };
          }
          if (returned && results_returned[fullId] === undefined) {
            results_returned[fullId] = { id: i, index: -1, dontValidate: true };
          }
        }
        query.inputs = [val];
        query.output = val;
        ...
        for (j = 0; j < nSearchWords; ++j) {
          ...
          if (
            !ty ||
            (filterCrates !== undefined && ty.crate !== filterCrates)
          ) {
            continue;
          }
          var lev_add = 0;
          ...
          if (in_args <= MAX_LEV_DISTANCE) {
            if (results_in_args[fullId] === undefined) {
              results_in_args[fullId] = { id: j, index: index, lev: in_args };
            }
          ...
          if (returned <= MAX_LEV_DISTANCE) {
            if (results_returned[fullId] === undefined) {
              results_returned[fullId] = { id: j, index: index, lev: returned };
            }
          ...
          if (
            ...
            if (results[fullId] === undefined) {
              results[fullId] = { id: j, index: index, lev: lev };
            }
    ...
    function getQuery(raw) {
      ...
      return { raw: raw, query: query, type: type, id: query + type };
    }
    ...
    function focusSearchResult() {
      var target =
        ...
        document
          .querySelectorAll("#titles > button")
    ...
    function showResults(results, go_to_first) {
      ...
      if (
        ...
        (results.others.length === 1 &&
          ...
          (!search.firstChild ||
            search.firstChild.innerText !== searchState.loadingText))
      ) {
    ...
    function execSearch(query, searchWords, filterCrates) {
      ...
      var results = { in_args: [], returned: [], others: [] };
    ...
    function registerSearchEvents() {
      ...
      document.getElementsByClassName("search-form")[0].onsubmit = function (
        e
      ) {
        e.preventDefault();
        searchState.clearInputTimeout();
